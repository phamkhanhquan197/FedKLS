...
class ServerSaveData:
    ...
    def fit_round(
        ...
        # Option 1: Print (đơn giản)
        print(f"Round {server_round}: Server parameters size: {len(self.parameters.tensors)}")
        print(f"Round {server_round}: Server parameters total size: {sum(len(p) for p in self.parameters.tensors)} bytes")
        
        # Option 2: Logging (có thể ghi file)
        ...
        # Option 3: Debugger (inspect variables)
        ...
        for client_proxy, fit_ins in client_instructions:
            # Option 1: Print
            print(f"Round {server_round}: Sending to client {client_proxy.cid}: {len(fit_ins.parameters.tensors)} parameter tensors")
            
            # Option 2: Logging
            # log(INFO, f"Round {server_round}: Sending to client {client_proxy.cid}: {len(fit_ins.parameters.tensors)} parameter tensors")
            
            # Verify parameters không None
        ...
        # DEBUG: Remove after debugging
        # Option 1: Print
        print(f"Round {server_round}: fit_clients() called with {len(client_instructions)} clients")
        print(f"Round {server_round}: Received {len(results)} results, {len(failures)} failures")
        ...
        for client_proxy, fit_res in results:
            ...
            # Option 1: Print
            print(f"Round {server_round}: Client {client_proxy.cid} returned {len(fit_res.parameters.tensors)} parameters, size: {param_size:.2f} MB")
        ...
        if parameters_aggregated is not None and parameters_aggregated.tensors:
            ...
            if old_param_hash == new_param_hash:
                # Option 1: Print
                print(f"WARNING: Round {server_round}: Server parameters did NOT change after aggregation!")
                ...
                print(f"Round {server_round}: Server parameters updated successfully ✓")
                # log(INFO, f"Round {server_round}: Server parameters updated successfully ✓")
        else:
            print(f"WARNING: Round {server_round}: parameters_aggregated is None or empty!")
