...
class SVDAdapter(nn.Module):
    def __init__(self, W_res, A, B, alpha, rank, original_bias=None):
        ...
        # IMPORTANT:
        # W_res must move with model.to(device). Register it as a buffer.
        # Do NOT force .cuda() here (breaks CPU/Ray/multi-device setups).
        self.register_buffer("W_res", W_res.clone().detach())
        self.W_res.requires_grad = False  # Freeze residual
    ...
    def __repr__(self):
        bias_info = (
            f", bias=None" if self.bias is None else f", bias={list(self.bias.shape)} (trainable: {self.bias.requires_grad})"
        )
        return (
...
class ConvAdapter(nn.Module):
    ...
    def __repr__(self):
        return (
            f"ConvAdapter(W_res: {list(self.W_res.shape)} (buffer, frozen), "
            f"A: {list(self.A.shape)} (trainable: {self.A.requires_grad}), "
            f"B: {list(self.B.shape)} (trainable: {self.B.requires_grad}), "
