...
class FedKLSVDClient(BaseClient):
    def __init__(
        self, client_id, model, trainset, valset, config_sim, device, save_dir, kl_norm, dataset=None, apply_transforms=None, data_scheduler=None, **kwargs
    ):
        super().__init__(client_id, model, trainset, valset, config_sim, device, save_dir, dataset=dataset, apply_transforms=apply_transforms, data_scheduler=data_scheduler, **kwargs)
    ...
    def __repr__(self) -> str:import torch
from flwr.common.logger import log
from logging import INFO
from collections import OrderedDict
from mak.clients.base_client import BaseClient
...
class FedKLSVDClient(BaseClient):
    def __init__(
        self, client_id, model, trainset, valset, config_sim, device, save_dir, kl_norm, dataset=None, apply_transforms=None, data_scheduler=None, bias=None
    ):
        super().__init__(client_id, model, trainset, valset, config_sim, device, save_dir, dataset=dataset, apply_transforms=apply_transforms, data_scheduler=data_scheduler, bias=bias)
        self.kl_norm = kl_norm  # Normalized KL divergence for this client
        # log(INFO, f"Client {self.client_id}: Initialized with kl_norm = {self.kl_norm}")
